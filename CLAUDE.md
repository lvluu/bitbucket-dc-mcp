...
**How it works:**
1. Push commits with [Conventional Commits](https://www.conventionalcommits.org/) format to `main`
   - `feat:` → minor version bump
   - `fix:` → patch version bump
   - `feat!:` or `BREAKING CHANGE:` → major version bump
2. On push to `main`, the `publish.yml` workflow runs release-please
3. release-please creates a **Release PR** with changelog and version bump
4. **Merge the Release PR** to trigger the actual release
5. The workflow then publishes to npm automatically

**Important:** Do NOT manually create version commits or tags. Let release-please handle it via the Release PR workflow.
